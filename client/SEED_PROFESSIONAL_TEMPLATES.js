/**
 * Professional Templates Seeding Script
 * This script creates and populates professional survey and event templates
...
async function seedSurveyTemplates() {
  console.log('🌱 Starting survey templates seeding...');
  
  try {
    // Get the first available user as template owner
    ...
    const templateOwnerId = users[0].id;
    let createdCount = 0;
    ...
    console.log(`🎉 Survey templates seeding completed! Created ${createdCount}/${surveyTemplates.length} templates.`);
...
async function seedEventTemplates() {
  ...
  try {
    ...
    console.log(`🎉 Event templates seeding completed! Created ${createdCount}/${eventTemplates.length} templates.`);
...
async function seedAllTemplates() {
  console.log('🚀 Starting professional templates seeding...');
  
  const surveyResult = await seedSurveyTemplates();
  const eventResult = await seedEventTemplates();
...
export { seedSurveyTemplates, seedEventTemplates, seedAllTemplates };

// Auto-run if this script is executed directly
if (typeof window !== 'undefined') {
  // Browser environment
  window.seedAllTemplates = seedAllTemplates;
  window.seedSurveyTemplates = seedSurveyTemplates;
  window.seedEventTemplates = seedEventTemplates;
  
  console.log('🔧 Template seeding functions are now available:');
  console.log('   - window.seedAllTemplates() - Seed all templates');
  console.log('   - window.seedSurveyTemplates() - Seed survey templates only');
  console.log('   - window.seedEventTemplates() - Seed event templates only');
} else {
  // Node.js environment
  seedAllTemplates().catch(console.error);
}
